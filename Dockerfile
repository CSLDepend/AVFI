...
# Install python3
RUN apt update && apt install -y build-essential bzip2 curl git python3-dev python3-opengl libfreetype6-dev python3-setuptools\
...
#RUN conda install -y -c anaconda setuptools numpy pyopengl

RUN python -m easy_install pip && python -m pip install pygame
RUN conda install --yes --file carla/PythonClient/requirements.txt

RUN cd carla/PythonClient && python setup.py build && python setup.py install
COPY . /av-il-fi
