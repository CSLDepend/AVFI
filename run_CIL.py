...
if (__name__ == '__main__'):
    ...
    ip_fi_list=[SolidOcclusion(0.7),WaterDrop(300,400,100,100,2.0,2.0),TransparentOcclusion(0.7),\
		SaltAndPepper(0.7),Gaussian(0.7),PassThrough(0.7)]
    op_fi_list=[ControlPassThrough(1),ControlDelayInjector(0.1)]
    ...
    vehicle_density=[20]
    ppl_density=[50]
    for ofm in op_fi_list:
        for cfm in ip_fi_list:
            print("TOP_LEVEL_DEBUG:",args.city_name,cfm.get_name())
            f_i=FaultInjector(cfm,ofm,args.city_name,args.dump_dashcam)
            agent.set_f_i(f_i)
            while True:
                try:
